...
export interface AutocompleteProps
	extends Omit<Partial<FieldInstance<"input">>, "onSelect" | "onChange" | "size"> {
	/**
...
export const Autocomplete = ({
	...
	const handleKeyDown = useCallback(
		...
		[activeSuggestionIndex, resetField, suggestions, value, handleSelect, onSelect]
	)
		...
		(item: string) => {
			...
			return parts.map((part, index) =>
				part.toLowerCase() === value?.toLowerCase() ? (
					<strong key={`${part}:${index}:highlighted`} style={{ color: setMatcher }}>
						{part}
					</strong>
				) : (
	...
	return (
		<div
			className={classNames([styles.Autocomplete, styles[`Autocomplete--${size}`]])}
			ref={autoCompleteRef}
