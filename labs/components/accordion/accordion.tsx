...
const AccordionMain = memo<AccordionProps>(
	({ title, children, size = "sm", className, dataKey, leadingIcon, trailingIcon }) => {
		...
		}, [dataKey, handleAccordionClick, title])
		...
		return (
			<motion.div className={accordionClassName}>
				<button
					className={classNames([styles.AccordionHeader, (isOpen && styles.isOpen) || ""])}
					onClick={onToggle}>
					<Flex fullWidth gap={8} alignItems="center">
						{leadingIcon && <span className={styles.leadingIcon}>{leadingIcon}</span>}

						{typeof title === "string" ? <Text.span weight={700}>{title}</Text.span> : title}
					</Flex>
...
const AccordionProvider = ({
	...
	useEffect(() => {
		if (defaultActiveKey) {
			setActiveKeys(allowMultiple ? [defaultActiveKey as number] : defaultActiveKey)
		}
...
const AccordionGroup = memo<AccordionGroupProps>(
	({ children, allowMultiple = false, defaultActiveKey }) => {
		return (
			<AccordionProvider allowMultiple={allowMultiple} defaultActiveKey={defaultActiveKey}>
				{children}
			</AccordionProvider>
		)
	}
