...
export interface IFieldDate
	extends Omit<Partial<FieldInstance<"input">>, "value" | "onChange" | "onSelect"> {
	/**
...
export const FieldDate: React.FC<IFieldDate> = ({
	...
	const { label, required, disabled, noAnimation, rounded, leadingIcon, placeholder } =
		props

	return (
		<AnimatePresence>
			<fieldset className={styles.FieldSet} disabled={disabled} aria-disabled={disabled}>
				{label && (
					<motion.label layout={!noAnimation} htmlFor={fieldId} className={styles.FieldLabel}>
				...
				<motion.div
					...
					<div className={classNames([styles.leadingIcon, "leading-icon"])} aria-hidden="true">
					...
					<DatePicker
						...
						onChange={onChange!}
						minDate={
							new Date(format(sub(Date.now(), { years: 100 }), "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"))
						}
