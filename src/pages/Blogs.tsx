  ...
  if (query.isLoading) return <Loader />
	...
	return (
			...
			<PaddedBlock>
				<section className="flex w-full flex-col items-center py-[99px] mb-[182px]">
					<div className="flex w-full flex-col items-center text-center">
						<h2 className="mb-10 mt-[10px] w-4/5 text-[32px] font-bold text-ash-300">
							Discover Articles and
							<span className="mx-1 text-secondary-200">
								Expert <br /> Perspectives
							</span>
							on AI
						</h2>
						<p className="w-3/4 font-work text-xl text-ash-200">
        ...
        <section className="w-full flex flex-col mb-[202px]">
          <p className="text-[28px] text-ash-300 font-bold mb-[51px]">
            Latest Articles
          </p>
            ...
            <div className="w-full">
              {posts?.data && (
                <Link to={`/blog/${posts.data[0]?.id}`} className="w-full flex flex-col gap-6">
                  ...
                  <h3 className="text-2xl text-ash-300 font-bold">{posts.data[0]?.title}</h3>
                  <p className="text-ash-200 font-work">{posts.data[0]?.content.substring(0, 200)}...</p>
              ...
              {posts?.data && posts.data.slice(1, 4).map((post) => (
                <Link key={post.id} to={`/blog/${post.id}`} className="w-full flex items-center gap-[33px]">
                  ...
                  <div className="flex flex-col gap-5">
                    <h3 className="text-xl text-ash-300 font-bold">{post.title}</h3>
                    <p className="text-ash-200 font-work">{post.content.substring(0, 70)}...</p>
                    <div className="flex items-center text-ash-200 font-work">
          ...
          <div className="w-full grid grid-cols-3 items-center gap-[47px] mb-[93px]">
            {posts?.data && posts.data.map((post) => (
              <Link key={post.id} to={`/blog/${post.id}`} className="w-full flex flex-col gap-5">
                ...
                <p className="text-xl text-ash-300 font-bold">
          ...
          <div className="w-full grid grid-cols-3 items-center gap-[41px]">
            {EVENTS.map((event, index) => (
              <div key={index} className="w-full flex flex-col items-center gap-5 text-center">
                ...
                <h4 className="text-xl text-ash-300 font-bold">{event.title}</h4>
                <div className="flex items-center text-ash-200 font-work">
