		...
		<>
			...
			<PaddedBlock>
				<section className="flex w-full flex-col py-[99px]">
					<p className="font-medium uppercase text-ash-200">what sets us apart</p>
					...
					<div className="grid w-full grid-cols-2 gap-x-[180px] gap-y-[120px]">
						{APPROACH.map((item, index) => (
							<div key={index} className="w-[387px]">
								...
								<p className="my-6 text-2xl font-bold text-ash-300">{item.label}:</p>
								<p className="font-work font-light text-ash-200">{item.decription}</p>
							</div>
						))}
					</div>
				</section>
				<section className="flex w-full flex-col items-center py-[99px]">
					<h2 className="w-1/2 text-center text-[32px] font-bold text-ash-300">
						...
						{SECTORS.map((sector, index) => (
							<button
								...
								className={`flex items-center justify-center rounded-[49px] px-8 py-[10px] capitalize ${
									tab === index ? "bg-secondary-100 text-black" : "bg-white/[8%] text-white"
								}`}>
								{sector.label}
							</button>
						))}
						...
						<TabPanel key={index} tabIndex={tab} index={index}>
							<div className="grid w-full grid-cols-2 items-center gap-[180px] md:w-[931px]">
								<div className="flex flex-col gap-6">
									<h5 className="text-2xl font-bold text-ash-300">{sector.topic}</h5>
									<p className="text-xl font-light text-ash-200">
										{sector.description}
									</p>
								</div>
